...
;(function(){
  $.queryToJson = function(qs, is_decode){
    ...
    for (i = 0, len = q_list.length ; i < len; i++) {
      if (q_list[i]) {
...
(function(){
    var cb = [];
    var isApp = /Lianjia/img.test(navigator.userAgent);
    var bridge = null;
    var bridgeKey = 'HybridBridgeLJ';
    var tokenKey = 'lianjia_token'; //放在cookie里的token
    //抛出来给别人用
    var bridgeInstance = null;

    var setBridgeInstance = function(){
      bridgeInstance = {};
      bridgeInstance.getDeviceId = function(){
        return (bridge['staticData'] && bridge['staticData']['deviceId']) || null;
      };
      bridgeInstance.getAppVersion = function(){
        return (bridge['staticData'] && bridge['staticData']['appVersion']) || null;
      };
      bridgeInstance.getNetwork = function(){
        return (bridge['staticData'] && bridge['staticData']['network']) || null;
      };
      bridgeInstance.getScheme = function(){
        return (bridge['staticData'] && bridge['staticData']['scheme']) || null;
      };
      bridgeInstance.getAccessToken = function(){
        return (bridge['staticData'] && bridge['staticData']['accessToken']) || null;
      };
      bridgeInstance.getUserInfo = function(){
        return (bridge['staticData'] && bridge['staticData']['userInfo']) || null;
      };
      bridgeInstance.getExtraData = function(){
        return (bridge['staticData'] && bridge['staticData']['extraData']) || null;
      };
      bridgeInstance.setPageTitle = function(title){
        try{
          bridge.setPageTitle && bridge.setPageTitle(title);
        }
        catch (e){

        }
      };
      bridgeInstance.actionLogin = function(url){
        try{
          bridge.actionLogin && bridge.actionLogin(url);
        }
        catch (e){
          
        }
      };
      bridgeInstance.actionWithUrl = function(pageId){
        try{
          bridge.actionWithUrl && bridge.actionWithUrl(pageId);
        }
        catch (e){
          
        }
      };
      bridgeInstance.setRightButton = function(params){
        try{
          bridge.setRightButton && bridge.setRightButton(params);
        }
        catch (e){
          
        }
      };
      bridgeInstance.setCity = function(cityId){
        try{
          bridge.setCity && bridge.setCity(cityId);
        }
        catch (e){
          
        }
      };
      bridgeInstance.setShareConfig = function(json){
        try{
          bridge.setShareConfig && bridge.setShareConfig(json);
        }
        catch (e){
          
        }
      };
      bridgeInstance.actionShare = function(){
        try{
          bridge.actionShare && bridge.actionShare();
        }
        catch (e){
          
        }
      };
      bridgeInstance.actionToast = function(str){
        try{
          bridge.actionToast && bridge.actionToast(str);
        }
        catch (e){
          
        }
      };
      bridgeInstance.pageActionBar = function(directive){
        try{
          bridge.pageActionBar && bridge.pageActionBar(directive);
        }
        catch (e){
    ...
    var run  = function(item) {
        setTimeout(function(){
            if(item.context) {
                item.fn.call(item.context, bridgeInstance, isApp);
            } else {
                item.fn(bridgeInstance, isApp);
            }
        }, 0);
    };
    // 初始化懒加载
    var init = function() {
        if(!isApp){
          bridge = {};
          setBridgeInstance();
          for (var i = 0, l = cb.length; i < l; i += 1) {
              run(cb[i]);
          }
          init = function(){};
          return;
        ...
        if (!bridge) {
            if(typeof window[bridgeKey] === 'undefined' || !window[bridgeKey]){
                setTimeout(init, 200);
                return;
            }
            bridge = window[bridgeKey];
            if(bridge._getStaticData) {
                bridge.staticData = JSON.parse(bridge._getStaticData());
            }
            //设置token的cookie
            if(bridge['staticData']['accessToken']){
              setCookie(tokenKey,bridge['staticData']['accessToken'],7);
            }
            //将特定方法传给bridgeInstance
            setBridgeInstance();
            for (var i = 0, l = cb.length; i < l; i += 1) {
                run(cb[i]);
            }
    ...
    $ljBridge = {};
    $ljBridge.ready = function(fn, context){
        if(bridgeInstance){
            if(context){
                fn.call(context, bridgeInstance, isApp);
            } else {
                fn(bridgeInstance, isApp);
            }
        }else{
            cb.push({fn:fn,context:context});
        }
      };
    init();
})();